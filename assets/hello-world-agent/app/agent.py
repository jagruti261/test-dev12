...
class SampleAgent:
    ...
    @tracer.start_as_current_span("hello_world_agent.run")
    async def _run_agent(
        ...
        """Execute the agent logic with full milestone instrumentation.

        Extracted from stream() so OpenTelemetry spans work correctly —
        using a context manager inside an async generator causes GeneratorExit errors.
        """
        # M2: Greeting flow — log that we received and are processing the user message
        logger.info("[M2.achieved]: greeting response initiated for context %s", context_id)
        ...
        # M3: Conversation loop complete
        logger.info("[M3.achieved]: conversation loop completed for context %s", context_id)
    ...
    async def stream(
        ...
        try:
            ...
            logger.error("[M2.missed]: greeting response not delivered for context %s", context_id)
            logger.error("[M3.missed]: conversation loop did not complete for context %s", context_id)
            yield {
                "is_task_complete": True,
                "require_user_input": False,
